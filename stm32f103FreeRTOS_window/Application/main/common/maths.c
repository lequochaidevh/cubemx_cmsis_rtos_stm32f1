...
static inline void sincosf_quadrant_r(float r, int q, float *out_s,
                                      float *out_c) {
  q &= 3;
...
// Initial implementation by Crashpilot1000
// (https://github.com/Crashpilot1000/HarakiriWebstore1/blob/396715f73c6fcf859e0db0f34e12fe44bace6483/src/mw.c#L1292)
// Polynomial coefficients by Andor
// (http://www.dsprelated.com/showthread/comp.dsp/21872-1.php) optimized by
// Ledvinap to save one multiplication Max absolute error 0,000027 degree
...
#define atanPolyCoef7 0.6444640676891548f
  ...
  if (res)
    ...
    res = 0.0f;
  res = -((((atanPolyCoef5 * res - atanPolyCoef4) * res - atanPolyCoef3) * res -
           atanPolyCoef2) *
              res -
          atanPolyCoef1) /
...
float acos_approx(float x) {
  ...
  float result = sqrtf(1.0f - xa) *
                 (1.5707288f +
                  xa * (-0.2121144f + xa * (0.0742610f + (-0.0187293f * xa))));
  if (x < 0.0f)
    return M_PIf - result;
  else
    return result;
...
float asin_approx(float x) { return (M_PIf * 0.5f) - acos_approx(x); }
...
void devClear(stdev_t *dev) { dev->m_n = 0; }
...
float devStandardDeviation(stdev_t *dev) { return sqrtf(devVariance(dev)); }

float degreesToRadians(int16_t degrees) { return degrees * RAD; }

int scaleRange(int x, int srcFrom, int srcTo, int destFrom, int destTo) {
  long int a = ((long int)destTo - (long int)destFrom) *
               ((long int)x - (long int)srcFrom);
  long int b = (long int)srcTo - (long int)srcFrom;
  return (a / b) + destFrom;
}

float scaleRangef(float x, float srcFrom, float srcTo, float destFrom,
                  float destTo) {
...
void arraySubInt32(int32_t *dest, const int32_t *array1, const int32_t *array2,
                   int count) {
...
int16_t qPercent(fix12_t q) { return (100 * q) >> 12; }

int16_t qMultiply(fix12_t q, int16_t input) { return (input * q) >> 12; }

fix12_t qConstruct(int16_t num, int16_t den) { return (num << 12) / den; }

// Cubic polynomial blending function
static float cubicBlend(const float t) { return t * t * (3.0f - 2.0f * t); }

// Smooth step-up transition function from 0 to 1
float smoothStepUpTransition(const float x, const float center,
                             const float width) {
