...
// only set_BASEPRI is implemented in device library. It does always create
// memory barrier missing versions are implemented here

// set BASEPRI register, do not create memory barrier
__attribute__((always_inline)) static inline void
__set_BASEPRI_nb(uint32_t basePri) {
  __ASM volatile("\tMSR basepri, %0\n" : : "r"(basePri));
}

// set BASEPRI_MAX register, do not create memory barrier
__attribute__((always_inline)) static inline void
__set_BASEPRI_MAX_nb(uint32_t basePri) {
...
static inline uint8_t __get_BASEPRI(void) { return atomic_BASEPRI; }
...
static inline void __basepriRestore(uint8_t *val) { atomic_BASEPRI = *val; }
...
// restore BASEPRI (called as cleanup function), with global memory barrier
static inline void __basepriRestoreMem(uint8_t *val) { __set_BASEPRI(*val); }
...
// restore BASEPRI (called as cleanup function), no memory barrier
static inline void __basepriRestore(uint8_t *val) { __set_BASEPRI_nb(*val); }
...
// Run block with elevated BASEPRI (using BASEPRI_MAX), restoring BASEPRI on
// exit. All exit paths are handled. Implemented as for loop, does intercept
// break and continue Full memory barrier is placed at start and at exit of
// block
...
#define ATOMIC_BLOCK(prio)                                                     \
  for (uint8_t __basepri_save __attribute__(                                   \
           (__cleanup__(__basepriRestoreMem), __unused__)) = __get_BASEPRI(),  \
                                              __ToDo =                         \
                                                  __basepriSetMemRetVal(prio); \
       __ToDo; __ToDo = 0)

// Run block with elevated BASEPRI (using BASEPRI_MAX), but do not create memory
// barrier. Be careful when using this, you must use some method to prevent
// optimizer from breaking things
// - lto is used for Cleanflight compilation, so function call is not memory
// barrier
// - use ATOMIC_BARRIER or volatile to protect used variables
// - gcc 4.8.4 does write all values in registers to memory before 'asm
// volatile', so this optimization does not help much
// - gcc 5 and later works as intended, generating quite optimal code
#define ATOMIC_BLOCK_NB(prio)                                                  \
  for (uint8_t __basepri_save __attribute__(                                   \
           (__cleanup__(__basepriRestore), __unused__)) = __get_BASEPRI(),     \
                                           __ToDo = __basepriSetRetVal(prio);  \
       __ToDo; __ToDo = 0)

// ATOMIC_BARRIER
// Create memory barrier
// - at the beginning of containing block (value of parameter must be reread
// from memory)
// - at exit of block (all exit paths) (parameter value if written into memory,
// but may be cached in register for subsequent use) On gcc 5 and higher, this
// protects only memory passed as parameter (any type can be used) this macro
// can be used only ONCE PER LINE, but multiple uses per block are fine
...
// based on
// https://stackoverflow.com/questions/24959440/rewrite-gcc-cleanup-macro-with-nested-function-for-clang
...
#define ATOMIC_BARRIER(data)                                                   \
  ...
  __cleanup_block __attribute__((cleanup(__do_cleanup), __unused__))           \
      __UNIQL(__cleanup) = ^{                                                  \
...
#define ATOMIC_BARRIER(data)                                                   \
  ...
  typeof(data) __attribute__((__cleanup__(__UNIQL(__barrierEnd)))) *           \
      __UNIQL(__barrier) = &data;                                              \
  ATOMIC_BARRIER_ENTER(__UNIQL(__barrier), #data);                             \
  do {                                                                         \
  } while (0) /**/
#endif
